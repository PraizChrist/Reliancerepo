...
if($tentants.GetType().IsArray -and $tentants.length -gt 1){
        ...
        for($subIdx=0; $subIdx -lt $tentants.length; $subIdx++){
                $opt = New-Object System.Management.Automation.Host.ChoiceDescription "$($tentants[$subIdx])", "Selects the $($tentants[$subIdx]) subscription."   
                $tentantsOptions.Add($opt)
        }
        $selectedSubIdx = $host.ui.PromptForChoice('Enter the desired Azure Subscription for this lab','Copy and paste the name of the subscription to make your choic', $tentantsOptions.ToArray(),0)
        try{
                $selectedtenant = $tentants[$selectedSubIdx] 
        }
        catch{
                 $selectedtenant = $tentants[$selectedSubIdx] | Select-Object -ExpandProperty Id
        }
        Write-Information "Selecting the $selectedtenant subscription"

}

# Get-PowerBIWorkspace -Scope Individual -FIlter "tolower(name) eq 'mps'"
$biuser = Login-PowerBIServiceAccount 
$subs = Get-PowerBIWorkspace -Scope Individual
if($subs.GetType().IsArray -and $subs.length -gt 1){
        $subOptions = [System.Collections.ArrayList]::new()
        for($subIdx=0; $subIdx -lt $subs.length; $subIdx++){
                $opt = New-Object System.Management.Automation.Host.ChoiceDescription "$($subs[$subIdx])", "Selects the $($subs[$subIdx]) subscription."   
        ...
        $selectedSubIdx = $host.ui.PromptForChoice('Enter the desired Azure Subscription for this lab','Copy and paste the name of the subscription to make your choic', $tentantsOptions.ToArray(),0)
        try{
                $selectedPBI = $subs[$selectedSubIdx] | Select-Object -ExpandProperty Id
                
        }
        catch{
                 $selectedPBI = $subs[$selectedSubIdx] 
        }
        Write-Information "Selecting the $selectedPBI subscription"
...
$powerbiLinkedName = "PowerBI$($uniqueId)"
$azurecongnitive = "asacognitive$($uniqueId)"
$pbigroupid  = $selectedPBI
...
$linkedServiceName = $powerbiLinkedName.ToLower()
...
try
{
    Execute-SQLScriptFile-SqlCmd -SQLScriptsPath $sqlScriptsPath -WorkspaceName $workspaceName -SQLPoolName $sqlPoolName -SQLUserName $sqlUserName -SQLPassword $sqlPassword -FileName "03_create_model" -Parameters $params
}
catch 
{
    write-host $_.exception
...
try
{
    Execute-SQLScriptFile-SqlCmd -SQLScriptsPath $sqlScriptsPath -WorkspaceName $workspaceName -SQLPoolName $sqlPoolName -SQLUserName $sqlUserName -SQLPassword $sqlPassword -FileName "04_sqlpool1_AggRow" -Parameters $params
}
catch 
{
    write-host $_.exception
